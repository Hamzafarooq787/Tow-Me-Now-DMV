          ...
          <div className="hidden lg:block relative">
            ...
            <div className="absolute bottom-10 right-10 z-20 glass-panel p-6 border-l-4 border-amber-brand">
              <div className="flex items-center gap-3">
                <span className="material-symbols-outlined text-amber-brand animate-pulse">location_on</span>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            <div>
              ...
              <p className="inter text-on-surface-variant max-w-2xl">
                Whatever the situation, our team is equipped with the latest technology and years of expertise to get you back on the road safely.
              </p>
            </div>
            <Link className="text-primary hover:text-amber-brand transition-colors font-bold uppercase tracking-widest text-sm flex items-center gap-2" href="/services">
              View All Services <span className="material-symbols-outlined">arrow_forward</span>
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: "car_crash", title: "Accident Management", desc: "Swift and careful vehicle removal from accident scenes. We handle the logistics so you can focus on what matters." },
              { icon: "rowing", title: "Recovery Towing", desc: "Specialized equipment for vehicles in ditches, mud, or complex recovery scenarios. Precision and power combined." },
              { icon: "minor_crash", title: "Roadside Assistance", desc: "Lockouts, jump-starts, tire changes, and fuel delivery. Small problems shouldn't ruin your entire day." },
            ].map(({ icon, title, desc }) => (
              <div key={title} className="bg-[#131929] p-10 border-l-4 border-amber-500 neon-border group transition-all duration-500">
                ...
                <p className="inter text-on-surface-variant leading-relaxed">{desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-[1280px] mx-auto px-8 grid lg:grid-cols-2 gap-16 items-center">
          <div className="relative">
            <div
              ...
              style={{ backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuBFzpwzZDhpddk1gf2lrN-_YgHcndzZKWxBkXYsJ2tFdo47XEqKVrVTktATCq-zhpexm4d0whnEfaSJQ15jNHCGKTYRSCXdbm5iKsr7kgc7MLYGJDu865kMeVcY1-LcVpKKCLV2r23bquW5iaKjyyYMiAAZM4evM-O39LXGY4Llj-4hI32fBsEEy-br3qUYFEsmOgkw5bPRzjgh-b1vZyNZ85Ww7VSbBvCiSb-Y-EkMJhp4nqEOBLbdGY3u1mUy5sNLAUAiL-3hYDM')" }}
            />
          ...
          <div>
            <h2 className="bebas text-5xl md:text-6xl text-on-background mb-8">
              Why Choose <span className="text-amber-brand">Tow Me Now DMV</span>?
            </h2>
            <div className="space-y-8">
              {[
                { icon: "verified", title: "Over 25 years of experience", desc: "Decades of navigating the DMV area means we know every shortcut and complication to get to you faster." },
                { icon: "engineering", title: "Skilled tow truck operators", desc: "Our team undergoes rigorous training and certification to handle any vehicle type, from luxury sedans to heavy trucks." },
                { icon: "schedule", title: "Available around the clock", desc: "Breakdowns don't wait for business hours. Neither do we. Rain or shine, day or night, we are your 24/7 lifeline." },
              ].map(({ icon, title, desc }) => (
                <div key={title} className="flex gap-6">
                  ...
                  <div>
                    ...
                    <p className="inter text-on-surface-variant">{desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section className="py-24 bg-surface">
        <div className="max-w-[1280px] mx-auto px-8">
          <div className="text-center mb-16">
            ...
            <p className="inter text-on-surface-variant">Real stories from drivers we&apos;ve helped across the region.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { quote: "Got a flat on I-495 at 2 AM. They arrived in less than 20 minutes. Professional, safe, and reasonably priced. Best towing experience I've had.", author: "MARCUS R.", span: 1 },
              { quote: "Excellent service after my accident. They handled my car with such care and even helped coordinate with my insurance. Highly recommend.", author: "SARAH T.", span: 1 },
              { quote: "Reliable and honest. The price they quoted over the phone was exactly what I paid. No hidden fees. Will definitely use them again.", author: "DAVID L.", span: 1 },
              { quote: "My battery died in the grocery parking lot. They were there fast and got me a jump start in seconds. Very polite crew!", author: "JESSICA K.", span: 1 },
              { quote: "If you're stranded in the DMV area, call these guys. They are the only ones I trust with my vintage BMW. They use flatbeds for everything and are extremely careful.", author: "ANTHONY V.", span: 2 },
            ].map(({ quote, author, span }) => (
              <div key={author} className={`glass-panel p-8 border-t-2 border-primary ${span === 2 ? "md:col-span-2" : "md:col-span-1"}`}>
                <div className="flex text-amber-brand mb-4">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <span key={i} className="material-symbols-outlined text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                  ))}
                </div>
                <p className="inter text-on-background italic mb-6">&ldquo;{quote}&rdquo;</p>
                <div className="font-bold text-primary bebas tracking-widest">— {author}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-surface-container-lowest" id="contact">
        <div className="max-w-[1280px] mx-auto px-8 grid lg:grid-cols-12 gap-16">
          <div className="lg:col-span-5">
            ...
            <p className="inter text-on-surface-variant mb-10 leading-relaxed">Fill out the form below for a non-emergency quote or general inquiry. For immediate emergency towing, please call us directly.</p>
            <div className="space-y-6">
              <div className="flex items-center gap-4 group">
                ...
                <div>
                  <p className="inter text-xs text-on-surface-variant uppercase tracking-widest">Emergency Dispatch</p>
                  <p className="bebas text-2xl text-on-background">202-812-0075</p>
                </div>
              </div>
              <div className="flex items-center gap-4 group">
                <div className="w-12 h-12 bg-primary/10 border border-primary/20 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                ...
                <div>
                  <p className="inter text-xs text-on-surface-variant uppercase tracking-widest">Email Us</p>
                  <p className="bebas text-2xl text-on-background">Towmenowdmv@gmail.com</p>
                </div>
              </div>
            </div>
          </div>
